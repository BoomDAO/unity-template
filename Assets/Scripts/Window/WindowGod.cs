namespace ItsJackAnton.UI
{
    ...
    public class WindowGod : Singleton<WindowGod>
    {
        readonly Dictionary<string, Window> openedWindows = new();
        [SerializeField, ShowOnly] private int unlockCursorWindowCount;

        // this invoke on window open of a specified type. "WILL INVOKE TRUE" if reopening a window
        public UnityBroadcast<(bool reopened, object data)> OnWindowOpen { get; private set; } = new();
        public UnityBroadcast OnWindowClose { get; private set; } = new();
        public UnityEvent<bool> OnEnablePlayerMovement { get; private set; } = new();
        public UnityEvent<bool> OnEnableCameraMovement { get; private set; } = new();
        ...
        private void HideConflictWindows(Window window)
        {
            ...
            if (windows != null)
            {
                for (int i = 0; i < windows.Length; i++)
                {
                    ...
                    {
                        if (hiddenWindows.TryGetValue(conflictWindowTypeName, out var conflictWindowList))
                        {
                            if (conflictWindowList.Contains(typeName) == false)
                            {
                                conflictWindow.gameObject.SetActive(false);
                                conflictWindowList.Add(typeName);
                            }
                        }
                        else
                        {
                            conflictWindow.gameObject.SetActive(false);
                            conflictWindowList = new List<string>();
                            conflictWindowList.Add(typeName);
                            hiddenWindows.Add(conflictWindowTypeName, conflictWindowList);
                        }
                    }
                }
            }
        ...
        public T OpenWindow<T>(object data) where T : Window
        {
            return OpenWindow<T>(typeof(T).Name, transform);
        }
        public T OpenWindow<T>(string WindowName, object data) where T : Window
        {
            void CheckIfCursorUnlockRequired(bool unlockCursor, bool countIfWindowFirstTime)
            {
                if (unlockCursor)
                {
                    ...
                    OnEnablePlayerMovement.Invoke(false);
                    OnEnableCameraMovement.Invoke(false);
                }
            }
            ...
            if (openedWindows.TryGetValue(typeName, out Window _baseWindow))
            {
                if (_baseWindow != null)
                {
                    window = (T)_baseWindow;
                    window.RectTransform.SetParent(transform);
                    ...
                    if (window.gameObject.activeSelf == false)
                    {
                        ...
                        //RemoveWindowEntryFromHiddenList(window);
                        HideConflictWindows(window);
                    }

                    OnWindowOpen.Invoke((true, data), window.GetType().FullName.ToHash16());
                    $"{typeof(T)} already Opened".Log<WindowGod>();
                    return window;
                }
            }

            window = Resources.Load<T>($"Windows/{typeName}");

            if (window == null)
            {
                $"Tried to open a window of name '{typeName}' but doesn't lives on Assets/Resources/Windows".Log<WindowGod>();
                return null;
            }
            ...
            window.Setup(data);
            //Add to Dictionary
            openedWindows.Add(typeName, window);

            CheckIfCursorUnlockRequired(window.RequireUnlockCursor(), true);

            HideConflictWindows(window);

            OnWindowOpen.Invoke((false, data), window.GetType().FullName.ToHash16());
        ...
        public T AddWidgets<T>(object data, Transform parent, Vector3 offset = default) where T : Window
        {
            ...
            if (parent == null)
            {
                $"Tried to add a widget of name '{typeNape}' but has no parent specified".Log<WindowGod>();
                return null;
            }
            ...
            if (window == null)
            {
                $"Tried to add a widget of name '{typeNape}' but doesn't lives on Assets/Resources/Widgets".Log<WindowGod>();
                return null;
            }
        ...
        public void CloseWindow(string typeName)
        {
            if (openedWindows.ContainsKey(typeName))
            {
                ...
                if (objInstance != null) objInstance.gameObject.Destroy();
                ...
                if (unlockCursorWindowCount == 0)
                {
                    if (CursorUnlockByWindowGod && !CursorUnlockByDefault)
                    {
                        ...
                        OnEnableCameraMovement.Invoke(true);
                    }
        ...
        public void CloseAllWindows()
        {
            ...
            {
                windows.Current.Value.gameObject.Destroy();
                windows.MoveNext();
            }
            openedWindows.Clear();
